...
import { getBestServer } from '/src/bestServer.js'
import { weakenServer } from '/src/parallel/weakenAlgo.js'
import { growServer } from '/src/parallel/growingAlgo.js'
import { hackServer } from '/src/parallel/hackingAlgo.js'
import { printServerStats } from '/src/serverStats.js'
import { buyServer, upgradeServer } from '/src/parallel/serverManager.js'
...
export async function main(ns) {
	...
	// for now, each has own server:	aws-0	aws-1	aws-2	aws-3
	//									weak	weak	grow	hack
	...
	while (true) {
		...
		let answerWeak1 = weakenServer(ns, target, "aws-0", 1)
		...
		let answerWeak2 = weakenServer(ns, target, "aws-1", 2)
		...
		let answerGrow = growServer(ns, target, "aws-2")

		// --------------------------------------
		// hacking

		// hacking start logic, for further time optimizations
		// note: when weak2 fails, the grow must also frail (and vice versa: when grow fails, weak2 shouldnt have started) 
		if (answerWeak1 == true && answerWeak2 == false && answerGrow == false) {
			...
			ns.print(yellow + "Weak 2 was skipped. Did the last hack attempt fail?\nHacking is about to start earlier than planned." + reset)
