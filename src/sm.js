...
export async function main(ns) {
	...
	if (upgrade) {
		...
		const answer = await ns.prompt("upgrading the server (" + upgradeName + ") to " + upgradeRam + "GB of Ram, will cost " + ns.formatNumber(price))
		...
		const answer = await ns.prompt("buying the server (" + buyName + ") with " + buyRam + "GB of Ram, will cost " + ns.formatNumber(price))
		...
		ramSize = ramSize / 2
		ns.tprint("you can buy a server with " + ramSize + "GB of ram. This costs " + ns.formatNumber(ns.getPurchasedServerCost(ramSize)))
