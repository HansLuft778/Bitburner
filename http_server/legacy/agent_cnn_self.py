...
"""
Try adding 3 or 4 convolutional layers with higher channel counts (e.g., 64 or 128 each), and increase the hidden size to 256 or 512. For example:

• 3-4 conv layers, each with 64+ out channels, all kernel_size=3 and padding=1
• A larger fully connected layer (e.g. hidden_size=256→512)
• Possibly add a batch norm or dropout layer for better generalization

This should boost model capacity without making training prohibitively slow on a 3080.
"""
...
class DQNAgentCNNShared:
    def __init__(
        ...
        if checkpoint_file != "" and checkpoint_file is not None:
            ...
            file = "models/checkpoints/" + checkpoint_file
    ...
    def select_action(
        ...
        """
        state_tensor = self.preprocess_state(board_state, history, is_white)
    ...
    def select_action_eval(self, board, valid_moves, game_history, color_is_white):
        """
