...
class TreePlot:
    ...
    def create_tree(self):
        ...
        def traverse(node: "Node", parent_idx: int | None = None):
            ...
            self.labels.append(
                f"win util: {node.win_utility:.5}<br>score util: {node.score_utility:.5}<br>util sum: {node.utility_sum:.5}<br>visit: {node.visit_cnt}<br>white: {node.is_white}<br>{rotate_and_beatify(node.uf.state)}"
            )
